...
public class LevelGen {
		...
		for (int j = 0; j < h; j++) {
			for (int x = 0; x < w; x++) {
				...
				if (map[x + j * w] != Tiles.get("snow").id && map[x + j * w] == Tiles.get("sand").id) {
					...
					int tx;
					check_biome_a: for (tx = x - tundra_desert_separator - random.nextInt(4); tx <= x + tundra_desert_separator + random.nextInt(4); tx++) {
	...
	public static void main(String[] args) {
		...
		while (!hasquit) { // stop the loop and close the program.) 
			...
			if ((w == 128) && (h == 128)) mapScale = 3;
			else if ((w == 256) && (h == 256)) mapScale = 2;
			else if ((w == 512) && (h == 512)) mapScale = 1;
