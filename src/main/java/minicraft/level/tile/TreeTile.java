...
public class TreeTile extends Tile {
    ...
    protected TreeTile(Tree type) {
        super((type == Tree.RED_MUSHROOM ? "Red mushroom" : (type == Tree.BROWN_MUSHROOM ? "Brown mushroom" : type.name() + " Tree")), (ConnectorSprite) null);
    	this.tree = type;
    ...
    public void hurt(Level level, int x, int y, int hurtDamage) {
        ...
        if (tree != Tree.BROWN_MUSHROOM && tree != Tree.RED_MUSHROOM) {
        	if (random.nextInt(50) == 25) {
        		level.dropItem(tx + 8, ty + 8, Items.get("Apple"));
        	}
        }

        Sound.playAt("genericHurt", tx, ty);
        level.add(new SmashParticle(tx, ty));

        level.add(new TextParticle("" + hurtDamage, tx + 8, ty + 8, Color.RED));

        if (damage >= treeHealth) {

        	level.dropItem(tx + 8, ty + 8, 2, 2, Items.get(tree.loot[0]));
            level.dropItem(tx + 8, ty + 8, 1, 2, Items.get(tree.loot[1]));
            level.dropItem(tx + 8, ty + 8, 0, 1, Items.get(tree.loot[2]));
    ...
    public void render(Screen screen, Level level, int x, int y) {
        ...
        if (u && ul && l) {
            tree.sprites[3].render(screen, x + 0, y + 0);
        } else {
        	tree.sprites[0].render(screen, x + 0, y + 0);
        }
        if (u && ur && r) {
        	tree.sprites[4].render(screen, x + 8, y + 0);
        } else {
        	tree.sprites[1].render(screen, x + 8, y + 0);
        }
        if (d && dl && l) {
        	tree.sprites[4].render(screen, x + 0, y + 8);
        } else {
        	tree.sprites[2].render(screen, x + 0, y + 8);
        }
        if (d && dr && r) {
        	tree.sprites[3].render(screen, x + 8, y + 8);
        } else {
        	tree.sprites[5].render(screen, x + 8, y + 8);
        }
    }
