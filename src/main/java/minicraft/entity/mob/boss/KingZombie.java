...
public class KingZombie extends EnemyMob {
     ...
     * 1, y - 17, textcol2); Font.draw(h, screen, (x - textwidth/2), y - 18,
     * textcol); }
     */

    public void die() {
        if (Settings.get("diff").equals("Peaceful"))
            dropItem(20, 20, Items.get("cloth"));
        if (Settings.get("diff").equals("Easy"))
            dropItem(20, 20, Items.get("cloth"));
        if (Settings.get("diff").equals("Normal"))
            dropItem(15, 15, Items.get("cloth"));
        if (Settings.get("diff").equals("Hard"))
            dropItem(10, 10, Items.get("cloth"));
        ...
        super.die();
        Updater.notifyAll("The zombie king has " + "been defeated!", 200);
    }
}
