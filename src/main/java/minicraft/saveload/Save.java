...
public class Save {
    ...
    private Save(File worldFolder) {
        ...
        if (worldFolder.getParent().equals("saves")) {
            ...
            if (!worldName.toLowerCase().equals(worldName)) {
                if (Game.debug)
                    System.out.println("Renaming world in " + worldFolder + " to lowercase");
    ...
    private void writeGame(String filename) {
        ...
        data.add(Settings.getIdx("mode")
                + (Game.isMode("score") ? ";" + Updater.scoreTime + ";" + Settings.get("scoretime") : ""));
    ...
    public static void writePlayer(Player player, List<String> data) {
        ...
        if (player.potioneffects.size() > 0)
            subdata = new StringBuilder(subdata.substring(0, subdata.length() - (1)) + "]"); // Cuts off extra ":" and
                                                                                             // appends "]"
        else
            subdata.append("]");
    ...
    public static String writeEntity(Entity e, boolean isLocalSave) {
        ...
        if (isLocalSave && (e instanceof ItemEntity || e instanceof Arrow || e instanceof RemotePlayer
                || e instanceof Spark || e instanceof Particle)) // wirte these only when sending a world, not writing
        ...
        } // The "else" part is so that remote player, which is a mob, doesn't get the
          // health thing.
            ...
            if (e instanceof EnemyMob)
                ...
                extradata.append(":").append(((Sheep) m).isCut); // Saves if the sheep is cut. If not, we could reload
                                                                 // the save and the wool would regenerate.
        }
        ...
        if (e instanceof Spawner) {
            ...
            extradata.append(":").append(mobname).append(":")
                    .append(egg.mob instanceof EnemyMob ? ((EnemyMob) egg.mob).lvl : 1);
        }
