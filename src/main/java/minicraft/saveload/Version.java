...
public class Version implements Comparable<Version> {
    ...
    private Version(String version, boolean printError) {
        ...
        try {
            if (nums.length > 0)
                make = Integer.parseInt(nums[0]);
            else
                make = 0;

            if (nums.length > 1)
                major = Integer.parseInt(nums[1]);
            else
                major = 0;
            ...
            if (printError)
                System.err.println("INVALID version number: \"" + version + "\"");
            valid = false;
        } catch (Exception ex) {
            if (printError)
                ex.printStackTrace();
    ...
    // the returned value of this method (-1, 0, or 1) is determined by whether this
    // object is less than, equal to, or greater than the specified object.
    public int compareTo(@NotNull Version ov) {
        if (make != ov.make)
            return Integer.compare(make, ov.make);
        if (major != ov.major)
            return Integer.compare(major, ov.major);
        if (minor != ov.minor)
            return Integer.compare(minor, ov.minor);
        if (dev != ov.dev) {
            if (dev == 0)
