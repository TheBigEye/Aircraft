...
public class Color {
    ...
    /**
     * This returns a minicraftrgb. a should be between 0-1, r,g,and b should be
     * 0-255
     */
    ...
    public static String toStringCode(int color) {
        return new String(new char[] { Color.COLOR_CHAR, (char) ((color >> 24) & 0xFF), // Alpha
    ...
    public static int get(String color) {
        ...
        return Color.get(color.charAt(leading), color.charAt(1 + leading), color.charAt(2 + leading),
                color.charAt(3 + leading));
    }
    ...
    public static int tint(int color, int amount, boolean isSpriteCol) {
        if (isSpriteCol) {
            int[] rgbBytes = separateEncodedSprite(color); // this just separates the four 8-bit sprite colors; they are
                                                           // still in base-6 added form.
    ...
    /**
     * seperates a 4-part sprite color (rgb4Sprite) into it's original 4 component
     * colors (which are each rgbBytes)
     */
    ...
    /**
     * This turns a 216 scale rgb int into a 0-5 scale "concatenated" rgb int. (aka
     * rgbByte -> r/g/b Readables)
     */
