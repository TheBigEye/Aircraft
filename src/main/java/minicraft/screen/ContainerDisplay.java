...
public class ContainerDisplay extends Display {
    ...
    public ContainerDisplay(Player player, Chest chest) {
        super(new InventoryMenu(chest, chest.getInventory(), chest.name),
    ...
    protected void onSelectionChange(int oldSel, int newSel) {
        ...
        if (oldSel == newSel)
            return; // this also serves as a protection against access to menus[0] when such may not
                    // exist.
        int shift = 0;
        if (newSel == 0)
    ...
    public void tick(InputHandler input) {
        ...
        if (curMenu.getNumOptions() > 0 && (input.getKey("attack").clicked || input.getKey("drop-one").clicked)) {
            ...
            boolean transferAll = input.getKey("attack").clicked || !(fromItem instanceof StackableItem)
                    || ((StackableItem) fromItem).count == 1;
