...
public class WorldSelectDisplay extends Display {
    ...
    public static void refreshWorldNames() {
        worldNames = null;
    }

    public static ArrayList<String> getWorldNames() {
        return getWorldNames(false);
    }
        ...
        for (File listOfFile : listOfFiles) {
            if (listOfFile.isDirectory()) {
                ...
                if (files != null && files.length > 0 && files[0].endsWith(Save.extension)) {
                    ...
                    worldNames.add(name);
                    if (Game.debug)
                        System.out.println("World found: " + name);
    ...
    public void init(Display parent) {
        ...
        menus = new Menu[] { new Menu.Builder(false, 0, RelPos.CENTER, entries).setDisplayLength(5)
                .setScrollPolicies(1, true).createMenu() };
    }
    ...
    public void render(Screen screen) {
        ...
        if (sel >= 0 && sel < worldVersions.size()) {
            ...
            Font.drawCentered(
                    Localization.getLocalized("World Version:")
                            + (version.compareTo(new Version("1.9.2")) <= 0 ? "~" : "") + version,
                    screen, Font.textHeight() * 7 / 2, col);
        }

        Font.drawCentered(Game.input.getMapping("select") + Localization.getLocalized(" to confirm"), screen,
