...
public class LinkEntry extends SelectEntry {
    ...
    // note that if the failMsg should be localized, such must be done before
    // passing them as parameters, for this class will not do it since, by default,
    // the failMsg contains a url.
        ...
        super(text, () -> {
            ...
            if (canBrowse) {
                ...
                try {
                    ...
                    Game.setMenu(new TempDisplay(3000, false, true, new Menu.Builder(true, 0, RelPos.CENTER,
                            new StringEntry(Localization.getLocalized(openMsg))).createMenu()));
                    desktop.browse(uri);
                } catch (IOException e) {
