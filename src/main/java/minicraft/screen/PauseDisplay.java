...
public class PauseDisplay extends Display {
    ...
    public PauseDisplay() {
        ...
        entries.addAll(Arrays.asList(new SelectEntry("Main Menu", () -> {
            ArrayList<ListEntry> items = new ArrayList<>();
            items.addAll(Arrays.asList(StringEntry.useLines("Are you sure you want to",
            ...
            items.add(new SelectEntry(Game.isValidServer() ? "Quit without saving" : "Yes",
                    () -> Game.setMenu(new TitleDisplay())));

            Game.setMenu(new Display(false, true, new Menu.Builder(true, 8, RelPos.CENTER, items).createMenu()));
        }),

                new BlankEntry(),

                new StringEntry(upString, Color.GRAY), new StringEntry(selectString, Color.GRAY)));

        menus = new Menu[] {
                new Menu.Builder(true, 4, RelPos.CENTER, entries).setTitle("Paused", Color.YELLOW).createMenu() };
    }
