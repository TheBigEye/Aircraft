...
public class LevelGen {
        ...
        do {
            ...
            for (int y = 0; y < h; y += stepSize) {
                for (int x = 0; x < w; x += stepSize) { // this loops through the values again, by a given increment...
                    ...
                    setSample(x + halfStep, y + halfStep, e); // this sets the value that is right in the middle of the
            ...
            for (int y = 0; y < h; y += stepSize) {
                for (int x = 0; x < w; x += stepSize) {
                    ...
                    // middle, right, mr-mb, mr-mt, and random.
                    double g = (a + c + d + f) / 4.0 + (random.nextFloat() * 2 - 1) * stepSize * scale * 0.5; // adds
                    // middle, bottom, mr-mb, ml-mb, and random.
    ...
    private void setSample(int x, int y, double value) {
         ...
         * does:
         * 
         * The values array is like a 2D array, but formatted into a 1D array; so the
         * basic "x + y * w" is used to access a given value.
         * 
         * The value parameter is a random number, above set to be a random decimal from
         * -1 to 1.
         * 
         * From above, we can see that the x and y values passed in range from 0 to the
     ...
     * random.setSeed(worldSeed); int attempt = 0; do { byte[][] result =
     * createHell(w, h);
     * 
     * int[] count = new int[256];
     * 
     * for (int i = 0; i < w * h; i++) { count[result[0][i] & 0xff]++; } if
     * (count[Tiles.get("Grass").id & 0xff] < 100) continue; if
     * (count[Tiles.get("Hard rock").id & 0xff] < 100) continue;
     * 
     * return result;
     * 
     * } while (true); }
    ...
    /**
     * private static byte[][] createHell(int w, int h) { LevelGen noise1 = new
     * LevelGen(w, h, 8); LevelGen noise2 = new LevelGen(w, h, 8);
     * 
     * byte[] map = new byte[w * h]; byte[] data = new byte[w * h];
     * 
     * for (int y = 0; y < h; y++) { for (int x = 0; x < w; x++) { int i = x + y *
     * w;
     * 
     * double val = Math.abs(noise1.values[i] - noise2.values[i]) * 3 - 2;
     * 
     * double xd = x / (w - 1.1) * 2 - 1; double yd = y / (h - 1.1) * 2 - 1; if (xd
     * < 0) xd = -xd; if (yd < 0) yd = -yd; double dist = xd >= yd ? xd : yd; dist =
     * dist * dist * dist * dist; dist = dist * dist * dist * dist; val = -val * 1 -
     * 2.2; val += 1 - dist * 2;
     * 
     * if (val < -0.35) { map[i] = Tiles.get("Hard rock").id; } else { map[i] =
     * Tiles.get("Grass").id; } } }
     * 
     * lavaLoop: for (int i = 0; i < w * h / 450; i++) { int x = random.nextInt(w -
     * 2) + 1; int y = random.nextInt(h - 2) + 1;
     * 
     * for (int yy = y - 1; yy <= y + 1; yy++) for (int xx = x - 1; xx <= x + 1;
     * xx++) { if (map[xx + yy * w] != Tiles.get("Hard Rock").id) continue lavaLoop;
     * }
     * 
     * Structure.lavaPool.draw(map, x, y, w); }
     * 
