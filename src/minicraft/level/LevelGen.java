...
public class LevelGen {
	...
	public LevelGen(int w, int h, int featureSize) {
		...
		/// feature size likely determines how big the biomes are, in some way. It tends
		...
		do {
			...
			/**
			 * THEN... this stuff is set to repeat the system all over again! The featureSize is halved, allowing access to further unset mids, and the scale changes... The scale increases the first time, x1.8, but the second time it's x1.1, and after that probably a little less than 1. So, it generally
			 * increases a bit, maybe to 4 / w at tops. This results in the 5th random value being more significant than the first 4 ones in later iterations.
			 */
			stepSize /= 2;
			scale *= (scaleMod + 0.8);
			scaleMod *= 0.4;
	...
	private void setSample(int x, int y, double value) {
		 ...
		 * From above, we can see that the x and y values passed in range from 0 to the width/height, and increment by a certain constant known as the "featureSize". This implies that the locations chosen from this array, to put the random value in, somehow determine the size of biomes, perhaps. The x/y
		 * value is taken and AND'ed with the size-1, which could be 127. This just caps the value at 127; however, it shouldn't be higher in the first place, so it is merely a safety measure.
		 * 
...
// Add trees to biomes ------------------------------------------------------------------------------------------------------------------------
		...
		if (Settings.get("Theme").equals("Plain")) {
			for (int i = 0; i < w * h / 2800; i++) {
				...
				int y = random.nextInt(h);
				for (int j = 0; j < 200; j++) {
					int xx = x + random.nextInt(15) - random.nextInt(15) + random.nextInt(5);
					int yy = y + random.nextInt(15) - random.nextInt(15) + random.nextInt(5);
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("grass").id) {
							map[xx + yy * w] = Tiles.get("tree").id;
						}
		...
		if (!Settings.get("Theme").equals("Plain")) {
			for (int i = 0; i < w * h / 400; i++) {
				...
				int y = random.nextInt(h);
				for (int j = 0; j < 200; j++) {
					int xx = x + random.nextInt(15) - random.nextInt(15) + random.nextInt(5) - random.nextInt(2);;
					int yy = y + random.nextInt(15) - random.nextInt(15) + random.nextInt(5) - random.nextInt(2);;
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("grass").id) {
							map[xx + yy * w] = Tiles.get("tree").id;
						}
...
// Sky dimension generation ===================================================================================================================
	...
	private static byte[][] createSkyMap(int w, int h) {
		...
		for (int i = 0; i < w * h / 2800; i++) {
			...
			for (int k = 0; k < 90; k++) {
				...
				for (int j = 0; j < 190; j++) {
					int xo = x + random.nextInt(5) - random.nextInt(5) + random.nextInt(2);
					int yo = y + random.nextInt(5) - random.nextInt(5) + random.nextInt(2);
					for (int yy = yo - 1; yy <= yo + 1; yy++)
						for (int xx = xo - 1; xx <= xo + 1; xx++)
							if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
		...
		for (int i = 0; i < w * h / 2800; i++) {
			...
			for (int k = 0; k < 100; k++) {
				int x = xs + random.nextInt(19) - random.nextInt(10) + random.nextInt(4);
				int y = ys + random.nextInt(19) - random.nextInt(10) + random.nextInt(4);
				for (int j = 0; j < 200; j++) {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			//byte[][] fullmap = LevelGen.createAndValidateUndergroundMap(w, h, 3);
