    ...
    public void render(Screen screen) {
        for (int x = 0; x < 200; x++) { // Loop however many times depending on the width (It's divided by 3 because the
            ...
            for (int y = 0; y < 150; y++) { // Loop however many times depending on the height (It's divided by 3
                ...
                if (dd < 0 && dd > -100) {
                    if (dt > 0)
                        screen.render(x * 8, y * 8, 30 + 30 * 32, 0, 3); // If the direction is upwards then render the
                                                                         // squares going up
                    else
                        screen.render(x * 8, Screen.h - y * 8 - 8, 30 + 30 * 32, 0, 3); // If the direction is negative,
