...
public class KeyInputDisplay extends Display {
    ...
    public KeyInputDisplay() {
        super(true);
        builder = new Menu.Builder(true, 1, RelPos.CENTER, getEntries()).setTitle("Controls")
                .setPositioning(new Point(Screen.w / 2, Screen.h - Font.textHeight() * 5), RelPos.TOP);

        Menu.Builder popupBuilder = new Menu.Builder(true, 4, RelPos.CENTER).setShouldRender(false)
                .setSelectable(false);

        menus = new Menu[] { builder.createMenu(),

                popupBuilder.setEntries(StringEntry.useLines(Color.YELLOW, "Press the desired", "key sequence"))
                        .createMenu(),

                popupBuilder.setEntries(StringEntry.useLines(Color.RED,
                        "Are you sure you want to reset all key bindings to the default keys?", "enter to confirm",
                        "escape to cancel")).setTitle("Confirm Action").createMenu() };
        ...
        if (confirmReset) {
            if (input.getKey("exit").clicked) {
                ...
                menus[0] = builder.setEntries(getEntries())
                        .setSelection(menus[0].getSelection(), menus[0].getDispSelection()).createMenu();
                selection = 0;
            }
    ...
    public void render(Screen screen) {
        if (selection == 0) // not necessary to put in if statement now, but it's probably more efficient
                            // anyway
            screen.clear(0);
        ...
        if (!listeningForBind && !confirmReset) {
            String[] lines = { "Press C/Enter to change key binding", "Press A to add key binding",
                    "Shift-D to reset all keys to default", Game.input.getMapping("exit") + " to Return to menu" };
