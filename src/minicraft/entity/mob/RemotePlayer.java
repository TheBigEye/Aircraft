...
public class RemotePlayer extends Player implements ClientTickable {
    ...
    /// these are used by the server to determine the distance limit for an
    /// entity/tile to be updated/added for a given player.
    ...
    public RemotePlayer(Player previous, InetAddress ip, int port) {
    ...
    public void clientTick() {
        ...
        if (hurtTime > 0)
            hurtTime--; // to update the attack animation.
    }
    ...
    public boolean move(int xa, int ya) {
        ...
        if (!(oldxt == x >> 4 && oldyt == y >> 4) && Game.isConnectedClient() && this == Game.player) {
            // if moved (and is client), then check any tiles no longer loaded, and remove
            // any entities on them.
    ...
    public void render(Screen screen) {
        super.render(screen);
        new FontStyle(Color.get(1, 204)).setShadowType(Color.BLACK, true).setXPos(x - Font.textWidth(username) / 2)
                .setYPos(y - 20).draw(username, screen); // draw the username of the player above their head
    }
    ...
    /// this determines if something at a given coordinate should be synced to this
    /// client, or if it is too far away to matter.
    ...
    public boolean shouldTrack(int xt, int yt, Level level) {
        return shouldSync(level, xt, yt, entityTrackingBuffer); /// this means that there is one tile past the syncRadii
                                                                /// in all directions, which marks the distance at which
                                                                /// entities are added or removed.
    }

    private boolean shouldSync(Level level, int xt, int yt, int offset) { // IDEA make this isWithin(). Decided not to
                                                                          // b/c different x and y radii.
        if (level == null)
            return false;
        if (getLevel() == null) {
            if (!Bed.inBed(this))
    ...
    public void updateSyncArea(int oldxt, int oldyt) {
        ...
        if (isServer) {
            ...
            System.err.println(
                    "ERROR: RemotePlayer sync method called when game is not client or server. Could be a disconnected client.");
            return;
        }

        /// the Math.mins and maxes make it so it doesn't try to update tiles outside of
        /// the level bounds.
        ...
        for (int y = ymin; y <= ymax; y++) {
            for (int x = xmin; x <= xmax; x++) {
                /// server loops through current tiles, and filters out old tiles, so only new
                /// tiles are left.
                /// client loops through old tiles, and filters out current tiles, so only old
                /// tiles are left.
                if (xt0 < 0 || yt0 < 0 || x > xt0 + xr || x < xt0 - xr || y > yt0 + yr || y < yt0 - yr) {

                    /// SERVER NOTE: don't worry about removing entities that go to unloaded tiles;
                    /// the client will do that. Now, as for mobs (or players) that wander out of or
                    /// into a player's loaded tiles without the player moving, the Mob class deals
                    /// with that.
