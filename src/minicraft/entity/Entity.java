...
public abstract class Entity implements Tickable {
	...
	/**
	 * Moves the entity a long only one direction. If xa != 0 then ya should be 0.
	 * If xa = 0 then ya should be != 0. Will throw exception otherwise.
	 * 
	...
	protected boolean move2(int xa, int ya) {
		...
		List<Entity> isInside = level
				.getEntitiesInRect(new Rectangle(x + xa, y + ya, xr * 2, yr * 2, Rectangle.CENTER_DIMS)); // gets the entities that this entity will touch once moved.
	...
	public final String getUpdates() {
		...
		for (int i = 0; i < curUpdates.length; i++) { // b/c the string always contains the same number of pairs (and
			// the same keys, in the same order), the indexes of cur and
			// prev updates will be the same.
