...
public class Updater extends Game {
	...
	public static void tick() {
		if (newMenu != menu) {
			...
			// if(debug) System.out.println("Setting menu from " + newMenu + " to " +
			// display);
		...
		if (Bed.sleeping() && !isValidClient()) {
			...
			if (gamespeed != 20) {
				...
				if (isValidServer()) {
					if (debug)
						System.out.println("SERVER: Setting time for bed");
					server.updateGameVars();
				}
			}
			if (tickCount > sleepEndTime) {
				if (Game.debug)
					System.out.println(Network.onlinePrefix() + "passing midnight in bed");
				pastDay1 = true;
				tickCount = 0;
				if (isValidServer())
					server.updateGameVars();
			...
			if (tickCount <= sleepStartTime && tickCount >= sleepEndTime) { // it has reached morning.
				if (Game.debug)
					System.out.println(Network.onlinePrefix() + "reached morning, getting out of bed");
				gamespeed = 1;
				if (isValidServer())
					server.updateGameVars();
				Bed.restorePlayers();
		...
		if (Renderer.canvas.hasFocus() || ISONLINE || !HAS_GUI) {
			...
			if (menu != null) {
				...
				if (!isValidServer()) {
					...
					// here is where I should put things like select up/down, backspace to boot, esc
					// to open pause menu, etc.
				...
				if (debug && HAS_GUI) {
					...
					if (!ISONLINE || isValidServer()) {
						...
						if (input.getKey("1").clicked)
							changeTimeOfDay(Time.Morning);
						if (input.getKey("2").clicked)
							changeTimeOfDay(Time.Day);
						if (input.getKey("3").clicked)
							changeTimeOfDay(Time.Evening);
						if (input.getKey("4").clicked)
						...
						if (input.getKey("shift-equals").clicked) {
							if (gamespeed < 1)
								gamespeed *= 2;
							else if (normSpeed * gamespeed < 2000)
								gamespeed++;
						}
						if (input.getKey("shift-minus").clicked) {
							if (gamespeed > 1)
								gamespeed--;
							else if (normSpeed * gamespeed > 5)
								gamespeed /= 2;
						}
					...
					if (!ISONLINE || isValidClient()) {
						...
						if (input.getKey("ctrl-h").clicked)
							player.health--;
						if (input.getKey("ctrl-b").clicked)
							player.hunger--;

						if (input.getKey("0").clicked)
							player.moveSpeed = 1;
						if (input.getKey("equals").clicked)
							player.moveSpeed++;
						if (input.getKey("minus").clicked && player.moveSpeed > 1)
	...
	public static void setTime(int ticks) {
		if (ticks < Time.Morning.tickTime)
			ticks = 0; // error correct
		if (ticks < Time.Day.tickTime)
			time = 0; // morning
		else if (ticks < Time.Evening.tickTime)
			time = 1; // day
		else if (ticks < Time.Night.tickTime)
			time = 2; // evening
		else if (ticks < dayLength)
			time = 3; // night
