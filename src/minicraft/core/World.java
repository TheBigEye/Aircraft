...
public class World extends Game {
    ...
    public static void initWorld() { // this is a full reset; everything.
        ...
        /*
         * if(isValidServer()) { System.err.
         * println("Cannot initialize world while acting as a server runtime; not running initWorld()."
         * ); return; }
         */
        ...
        Updater.changeTimeOfDay(Updater.Time.Morning); // resets tickCount; game starts in the day, so that it's nice
                                                       // and bright.
        gameOver = false;
        ...
        if (!isValidClient()) {
            ...
            if (WorldSelectDisplay.loadedWorld()) {
                ...
                float loadingInc = 100f / (maxLevelDepth - minLevelDepth + 1); // the .002 is for floating point errors,
                                                                               // in case they occur.
                for (int i = maxLevelDepth; i >= minLevelDepth; i--) {
                    // i = level depth; the array starts from the top because the parent level is
                    // used as a reference, so it should be constructed first. It is expected that
                    // the highest
                    // level will have a null parent.
                    if (debug)
                        System.out.println("Loading level " + i + "...");
                    LoadingDisplay.setMessage(Level.getDepthString(i));
                    levels[lvlIdx(i)] = new Level(worldSize, worldSize, WorldGenDisplay.getSeed(), i,
                            levels[lvlIdx(i + 1)], !WorldSelectDisplay.loadedWorld());
