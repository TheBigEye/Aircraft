...
public class World extends Game {
	...
	public static final int[] idxToDepth = { -3, -2, -1, 0, 1, -4 }; /// this is to map the level depths to each level's index in Game's levels array. This must ALWAYS be the same length as the
												 /// levels array, of course.
	...
	public static int lvlIdx(int depth) {
		if (depth > maxLevelDepth)
			return lvlIdx(minLevelDepth);
		if (depth < minLevelDepth)
			return lvlIdx(maxLevelDepth);
	...
	/**
	 * This method is used to create a brand new world, or to load an existing one from a file. For the loading screen updates to work, it it assumed that *this* is called by a thread *other* than
	 * the one rendering the current *menu*.
	 **/
	public static void initWorld() { // this is a full reset; everything.
		...
		/*
		 * if(isValidServer()) { System.err.println("Cannot initialize world while acting as a server runtime; not running initWorld()."); return; }
		 */
		...
		if (!isValidClient()) {
			if (debug)
				System.out.println("Initializing world non-client...");
			...
			if (WorldSelectDisplay.loadedWorld()) {
				...
				for (int i = maxLevelDepth; i >= minLevelDepth; i--) {
					// i = level depth; the array starts from the top because the parent level is used as a reference, so it should be constructed first. It is expected that the highest
					// level will have a null parent.
					if (debug)
	...
	/**
	 * This method changes the level that the player is currently on. It takes 1 integer variable, which is used to tell the game which direction to go. For example, 'changeLevel(1)' will make you
	 * go up a level, while 'changeLevel(-1)' will make you go down a level.
	 */
