...
public class MinicraftServer extends Thread implements MinicraftProtocol {
    ...
    public String[] getClientInfo() {
        ...
        for (MinicraftServerThread serverThread : getThreads()) {
            ...
            playerStrings.add(clientPlayer.getUsername() + ": " + clientPlayer.getIpAddress().getHostAddress()
                    + (Game.debug ? " (" + (clientPlayer.x >> 4) + "," + (clientPlayer.y >> 4) + ")" : ""));
        }
    ...
    public List<RemotePlayer> getPlayersInRange(Entity e, boolean useTrackRange) {
        ...
        return getPlayersInRange(e.getLevel(), xt, yt, useTrackRange); // NOTE if "e" is a RemotePlayer, the list
                                                                       // returned *will* contain "e".
    }
    ...
    private List<MinicraftServerThread> getAssociatedThreads(List<RemotePlayer> players) {
        ...
        // NOTE I could do this the other way around, by looping though the thread list,
        // and adding those whose player is found in the given list, which might be
        // slightly more optimal... but I think it's better that this tells you when a
        // player in the list doesn't have a matching thread.
    ...
    public void broadcastEntityUpdate(Entity e) {
        broadcastEntityUpdate(e, false);
    }

    public void broadcastEntityUpdate(Entity e, boolean updateSelf) {
        if (e.isRemoved()) {
            if (Game.debug)
                System.out.println("SERVER: Tried to broadcast update of removed entity: " + e);
            return;
        }
    ...
    public void broadcastEntityAddition(Entity e) {
        broadcastEntityAddition(e, false);
    }

    public void broadcastEntityAddition(Entity e, boolean addSelf) {
        if (e.isRemoved()) {
            if (Game.debug)
                System.out.println("SERVER: Tried to broadcast addition of removed entity: " + e);
            return;
        }
        ...
        if (Game.debug && e instanceof Player)
            System.out.println("SERVER: Broadcasting player addition of " + e);
        for (MinicraftServerThread thread : getAssociatedThreads(players)) {
            thread.sendEntityAddition(e);
            cnt++;
        }
        if (Game.debug && e instanceof Player)
            System.out.println("SERVER: Broadcasted player addition of " + e + " to " + cnt + " clients");
    }
    ...
    private List<RemotePlayer> getPlayersToRemove(Entity e, boolean removeSelf) {
        ...
        if (players.size() == 0)
            return players;
        if (Game.debug && e instanceof Player) {
            System.out.println("SERVER: Sending removal of player " + e + " to " + players.size()
                    + " players (may remove equal player): ");
        ...
        if (Game.debug && e instanceof Player)
            System.out.println("...now sending player removal to " + players.size() + " players.");
    ...
    public void broadcastEntityRemoval(Entity e, Level level, boolean removeSelf) {
        ...
        if (level == null) {
            if (Game.debug)
                System.out.println("SERVER: Cannot remove entity " + e
                        + " from specified level, level given is null; ignoring request to broadcast entity removal.");
            return;
        }
    ...
    public void updateGameVars(MinicraftServerThread[] sendTo) {
        ...
        String[] varArray = { Settings.get("mode").toString(), Updater.tickCount + "", Updater.gamespeed + "",
                Updater.pastDay1 + "", Updater.scoreTime + "", getNumPlayers() + "", Bed.getPlayersAwake() + "" };
    ...
    boolean parsePacket(MinicraftServerThread serverThread, InputType inType, String alldata) {
        ...
        if (clientPlayer == null) {
            System.err.println("CRITICAL SERVER ERROR: Server thread client is null: " + serverThread
        ...
        if (inType == InputType.INVALID) {
            if (Game.debug)
                System.out.println(serverThread + " received an error:");
        ...
        if (InputType.serverOnly.contains(inType)) {
            ...
            System.err.println(
                    "SERVER WARNING: Client " + clientPlayer + " sent illegal packet type " + inType + " to server.");
            return false;
        }
        ...
        case LOGIN:
            ...
            if (Game.debug)
                System.out.println("SERVER: Login data: " + Arrays.toString(data));
            ...
            // check if the same username already exists on the server (due to signing on a
            // second time with the same account), and if so, prevent the new login
            ...
            // now, we need to check if this player has played in this world before. If they
            // have, then all previous settings and items and such will be restored.
            ...
            if (serverThread.getClient().getIpAddress().isLoopbackAddress() && hostPlayer == null) {
                // this is the first person on localhost. I believe that a second person will be
                // saved as a loopback address (later: jk the first one actually will), but a
                // third will simply overwrite the second.
                if (Game.debug)
                    System.out.println("SERVER: Host player found");

                if (Game.player != null) {
                    // save the player, and then remove it. It is leftover from when this was a
                    // single player world.
            ...
            if (playerdata.length() > 0) {
                // if a save file was found, then send the data to the client so they can resume
                // where they left off.
                // and now, initialize the RemotePlayer instance with the data.
                // first get the version that saved the file, which can be different if this is
                // a remote player.
            ...
            if (!Arrays.asList(World.levels[playerlvl].getEntityArray()).contains(clientPlayer)
                    && clientPlayer != hostPlayer) // this will be true if their file was already found, since they are
                                                   // added in Load.loadPlayer().
                World.levels[playerlvl].add(clientPlayer); // add to level (**id is generated here**) and also, maybe,
                                                           // broadcasted to other players?

            updateGameVars();
            // making INIT_W packet
            int[] toSend = { clientPlayer.eid, World.levels[playerlvl].w, World.levels[playerlvl].h, playerlvl, // these
                                                                                                                // bottom
                                                                                                                // three
                                                                                                                // are
                                                                                                                // actually
                                                                                                                ...
                                                                                                                // PLAYER
                                                                                                                // packet.
                    clientPlayer.x, clientPlayer.y };
            StringBuilder sendString = new StringBuilder();
            for (int val : toSend)
                sendString.append(val).append(",");
            // send client world info
            if (Game.debug)
                System.out.println("SERVER: Sending INIT packet");
            serverThread.sendData(InputType.INIT, sendString.toString());
            return true;

        case LOAD:
            if (Game.debug)
                System.out.println("SERVER: Received level data request");
            ...
            if (levelidx < 0 || levelidx >= World.levels.length) {
                System.err.println("SERVER warning: Client " + clientPlayer
                        + " tried to request tiles from nonexistent level " + levelidx);
            ...
            // move the associated player to the level they requested -- they shouldn't be
            // requesting it if they aren't going to transfer to it.
            // moved to after the tile data is sent so that the client doesn't try to add
            // anything to the level before it gets created.
            Level next = World.levels[levelidx];
            if (clientPlayer.getLevel() != null && !next.getTile(clientPlayer.x >> 4, clientPlayer.y >> 4).mayPass(next,
                    clientPlayer.x >> 4, clientPlayer.y >> 4, clientPlayer))
            ...
            for (Entity curEntity : entities) {
                if (!clientPlayer.shouldTrack(curEntity.x >> 4, curEntity.y >> 4, curEntity.getLevel()))
                    continue; // this is outside of the player's entity tracking range; doesn't need to know
                              // about it yet.
                ...
                if (curEntityData.length() > 1) // 1 b/c of the "," added; this prevents entities that aren't saved from
                                                // causing ",," to appear.
        ...
        case DIE:
            if (Game.debug)
                System.out.println("Received player death");
        ...
        case ENTITY:
            // client wants the specified entity sent in an ADD packet, becuase it couldn't
            // find that entity upon receiving an ENTITY packet from the server.
            int enid = Integer.parseInt(alldata);
            Entity entityToSend = Network.getEntity(enid);
            if (entityToSend == null) {
                /// well THIS would be a problem, I think. Though... Actually, not really. It
                /// just means that an entity was removed between the time of sending an update
                /// for it, and the client then asking for it to be added. But since it would be
                /// useless to add it at this point, we'll just ignore the request.
                if (Game.debug)
                    System.out.println(
                            "SERVER: Ignoring request to add unknown entity (probably already removed): " + enid);
                return false;
            }
            ...
            if (Game.debug)
                System.out.println("SERVER: Sending entity addition via " + serverThread
                        + " because client requested it: " + entityToSend);
            serverThread.sendEntityAddition(entityToSend);
            return true;

        case SAVE:
            if (Game.debug)
                System.out.println("SERVER: Received player save from " + serverThread.getClient());
            // save this client's data to a file.
            // first, determine if this is the main player. if not, determine if a file
            // already exists for this client. if not, find an available file name. for
            // simplicity, we will just count the number of remote player files saved.

            if (clientPlayer == hostPlayer) {
                if (Game.debug)
                    System.out.println("SERVER: Identified SAVE packet client as host");
        ...
        case CHESTOUT:
            ...
            if (!(e instanceof Chest)) {
                System.err.println(
                        "SERVER: Error with CHESTOUT request: Specified chest entity did not exist or was not a chest.");
                return false;
            }
            ...
            if (inType == InputType.CHESTIN) {
                ...
                if (item instanceof UnknownItem) {
                    System.err.println(
                            "SERVER error with CHESTIN request: Specified item could not be found from string: "
                                    + data[2]);
                    return false;
                }
                ...
                if (itemIdx >= chest.getInventory().invSize() || itemIdx < 0) {
                    System.err
                            .println("SERVER: Error with CHESTOUT request: Specified chest inv index is out of bounds: "
                                    + itemIdx + "; inv size:" + chest.getInventory().invSize());
                    return false;
                }
                ...
                serverThread.sendData(InputType.CHESTOUT, itemToSend.getData() + ";" + data[3]); // send back the item
                                                                                                 // that the player
                                                                                                 // should put in their
                                                                                                 // inventory.
            }
        ...
        case PUSH:
            ...
            if (furniture == null) {
                ...
                System.err.println("SERVER: Specified entity is not an instance of the furniture class: " + furniture
                        + "; cannot push.");
                return false;
            }
        ...
        case PICKUP:
            ...
            if (!(entity instanceof ItemEntity) || entity.isRemoved()) {
                System.err.println("SERVER: Could not find item entity in pickup request " + ieid
                        + ". Telling client to remove...");
                serverThread.sendEntityRemoval(ieid); // will happen when another guy gets to it first, so the this
                                                      // client shouldn't have it on the level anymore. It could also
                                                      // happen if the client didn't receive the packet telling them to
                                                      // pick it up... in which case it will be lost, but oh well.
                return false;
            }
        ...
        case INTERACT:
            ...
            clientPlayer.attack(); /// NOTE the player may fire an arrow, but we won't sync the arrow count because
                                   /// that player will update it theirself.

            serverThread.sendData(InputType.INTERACT,
                    (clientPlayer.activeItem == null ? "null" : clientPlayer.activeItem.getData()));
            return true;

        case BED:
            if (Game.debug)
                System.out.println("Received bed request: " + alldata);
            ...
            if (getIn) {
                ...
                if (Bed.sleeping())
                    return false; // can't quit once everyone is in bed
        ...
        case MOVE:
            ...
            boolean moved = clientPlayer.move(newx - clientPlayer.x, newy - clientPlayer.y); // this moves the player,
                                                                                             // and updates other
                                                                                             // clients.

            clientPlayer.dir = Direction.values[Integer.parseInt(data[2])]; // do this AFTERWARD, so that the move
                                                                            // method doesn't mess something up.

            if (moved)
                clientPlayer.updateSyncArea(oldx, oldy); // this updates the current client.

            broadcastEntityUpdate(clientPlayer, !moved); // this will make it so that if the player is prevented from
                                                         // moving, the server will update the client, forcing it back
                                                         // to the last place the server recorded the player at. TODO
                                                         // this breaks down with a slow connection...
            clientPlayer.walkDist++; // hopefully will make walking animations work. Actually, they should be sent
                                     // with Mob's update... no, it doesn't update, it just feeds back.

            return true;

        // I'm thinking this should end up never being used... oh, well maybe for
    ...
    public synchronized void endConnection() {
        ...
        if (Game.debug)
            System.out.println("SERVER: Ending connection with threads: " + threadList);
