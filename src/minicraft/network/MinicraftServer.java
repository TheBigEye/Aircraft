...
public class MinicraftServer extends Thread implements MinicraftProtocol {
	...
	public void broadcastEntityRemoval(Entity e, Level level, boolean removeSelf) {
		...
		if (level == null) {
			if (Game.debug)
				System.out.println("SERVER: Cannot remove entity " + e + " from specified level, level given is null; ignoring request to broadcast entity removal.");
			return;
		}
	...
	public void updateGameVars(MinicraftServerThread[] sendTo) {
		...
		String[] varArray = { Settings.get("mode").toString(), Updater.tickCount + "", Updater.gamespeed + "", Updater.pastDay1 + "", Updater.scoreTime + "", getNumPlayers() + "",
				Bed.getPlayersAwake() + "" };
	...
	boolean parsePacket(MinicraftServerThread serverThread, InputType inType, String alldata) {
		...
		case LOGIN:
			...
			int[] toSend = { clientPlayer.eid, World.levels[playerlvl].w, World.levels[playerlvl].h, playerlvl, // these bottom three are actually unnecessary because of the previous PLAYER packet.
		...
		case ENTITY:
			...
			if (entityToSend == null) {
				...
				if (Game.debug)
					System.out.println("SERVER: Ignoring request to add unknown entity (probably already removed): " + enid);
				return false;
			}
			...
			if (Game.debug)
				System.out.println("SERVER: Sending entity addition via " + serverThread + " because client requested it: " + entityToSend);
