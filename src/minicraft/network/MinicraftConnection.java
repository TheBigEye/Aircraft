...
public abstract class MinicraftConnection extends Thread implements MinicraftProtocol {
    ...
    public void run() {
        ...
        while (isConnected()) {
            ...
            try {
                ...
                System.err.println(
                        this + " had a problem reading its input stream (will continue trying): " + ex.getMessage());
                ex.printStackTrace();
            }

            if (read < 0) {
                if (Game.debug)
                    System.out.println(this + " reached end of input stream.");
                break;
            }

            // if (Game.debug) System.out.println(this + " successfully read character from
            // input stream: " + read);

            if (read > 0) { // if it is valid character that is not the null character, then add it to the
                            // string.
                currentData.append((char) read);
            } else if (currentData.length() > 0) { // read MUST equal 0 at this point, aka a null character; the if
                                                   // statement makes it ignore sequential null characters.

                // if (Game.debug) System.out.println(this + " completed data packet: " +
                // currentData);
        ...
        if (Game.debug)
            System.out.println("run loop ended for " + this + "; ending connection.");
    ...
    protected void sendData(InputType inType, String data) {
        ...
        if (Game.packet_debug && Game.isConnectedClient())
            System.out.println("Sent:" + inType.toString() + ", " + data);

        char inTypeChar = (char) (inType.ordinal() + 1);
        // if (Game.debug && inType == InputType.TILES) System.out.println(this + ":
        // printing " + inType + " data:");
        if (data.contains("\0"))
            System.err.println("WARNING from " + this + ": data to send contains a null character. Not sending data.");
        else {
    ...
    @NotNull
    public static String stringToInts(String str) {
        return stringToInts(str, str.length());
    }
    ...
    /// there are a couple methods that are identical in both a server thread, and
    /// the client, so I'll just put them here.
    ...
    public void endConnection() {
        if (socket != null && !socket.isClosed()) {
            if (Game.debug)
                System.out.println("closing socket and ending connection for: " + this);
