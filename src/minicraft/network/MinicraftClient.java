...
public class MinicraftClient extends MinicraftConnection {
	...
	public boolean parsePacket(InputType inType, String alldata) {
		...
		case TILES:
			...
			if (tiledata.length / 2 > level.tiles.length) {
				System.err.println("CLIENT ERROR: received level tile data is too long for world size; level.tiles.length=" + level.tiles.length + ", tiles in data: " + (tiledata.length / 2)
		...
		case REMOVE:
			...
			if (toRemove != null) {
				if (entityLevelDepth != null && toRemove.getLevel() != null && toRemove.getLevel().depth != entityLevelDepth) {
					if (Game.debug)
						System.out.println("CLIENT: not removing entity " + toRemove + " because it is not on the specified level depth, " + entityLevelDepth + "; current depth = "
								+ toRemove.getLevel().depth + ". Removing from specified level only...");
	...
	public void requestLevel(int lvlidx) {
		if (Game.debug)
			System.out.println("CLIENT: setting level before request to be sure, from " + Game.currentLevel + " to " + lvlidx);
